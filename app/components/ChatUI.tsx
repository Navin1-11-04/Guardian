...
function isDeleteConfirm(data: unknown): data is DeleteConfirm {
  return (
    typeof data === "object" && data !== null && "confirmation_required" in data
  );
}
    ...
    <div className="rounded-xl border border-foreground/10 overflow-hidden">
      ...
      <div className="px-4 py-3 grid grid-cols-2 gap-2.5">
        {[
          ...
          },
          {
            icon: <Hash size={12} />,
            label: "Forks",
            value: String((data as any).forks ?? 0),
          },
          {
          ...
          },
          {
            icon: <Calendar size={12} />,
            label: "Created",
            value: timeAgo((data as any).created_at),
          },
      ...
      {(data as any).branches?.length > 0 && (
        <div className="px-4 py-2.5 border-t border-foreground/5">
          <div className="text-[10px] text-foreground/30 uppercase tracking-wider mb-1.5">
            Branches
          </div>
          <div className="flex flex-wrap gap-1.5">
            {(data as any).branches.map((b: string) => (
              <span
                key={b}
                className="text-[10px] px-2 py-0.5 rounded bg-foreground/5 text-foreground/50 border border-foreground/10 font-mono"
              >
                {b}
              </span>
            ))}
...
function JsonCard({ raw }: { raw: string }) {
  ...
  if (isDeleteConfirm(data))
    return (
      <div className="mt-3">
        <DeleteConfirmCard data={data as DeleteConfirm} />
      </div>
    );
  ...
  if (isDeleteResult(data))
    return (
      <div className="mt-3">
        <DeleteResultCard data={data as DeleteResult} />
      </div>
    );
  ...
  if (isCreatedIssue(data))
    return (
      <div className="mt-3">
        <CreatedIssueCard data={data as CreatedIssue} />
      </div>
    );
...
export default function ChatUI({ user }: { user: User }) {
  ...
  useEffect(() => {
    fetch("/api/policies")
      .then((r) => r.json())
      .then((d) => setPolicies(Array.isArray(d.policies) ? d.policies : []));
    fetch("/api/audit")
      .then((r) => r.json())
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <header className="w-full relative z-999 bg-transparent flex items-center justify-between px-4 py-3">
        <div className="flex items-center gap-2.5 pointer-events-none">
          ...
          <div className="leading-tighter">
            ...
            <div className="text-[10px] text-foreground/50 tracking-wide uppercase font-medium">
              AI Agent Firewall
            </div>
          </div>
        </div>
            ...
            {messages.length === 0 ? (
              <div className="text-center py-10">
                ...
                <p className="text-sm text-foreground/50 max-w-2xl mx-auto mb-6 leading-tight">
                  Every action is intercepted by Guardian and evaluated against
                  your security policies before execution.
                </p>
      ...
      {showSheet && (
        <>
          <div
            className="fixed inset-0 bg-foreground/10 z-999 backdrop-blur-[3px]"
                    ...
                    {policies.map((policy) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex gap-2">
                            {(["allow", "block", "step-up"] as const).map(
                              (d) => {
